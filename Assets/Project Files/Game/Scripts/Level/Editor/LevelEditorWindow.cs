...
namespace Watermelon
{
    public class LevelEditorWindow : LevelEditorBase
    {
        ...
        protected override void InitializeVariables()
        {
            ...
            tabHandler.AddTab(new TabHandler.Tab(SETTINGS_TAB_NAME, DisplayPropertiesTab));
        ...
        protected override void Styles()
        {
            ...
            if (tabHandler != null)
            {
                ...
                editColorsContent = new GUIContent(EditorCustomStyles.GetIcon("icon_menu"), "Edit used colors");
            }
        }
        ...
        private void DisplaySidebar()
        {
            ...
            if(GUILayout.Button("Clear selection", EditorCustomStyles.button))
            {
                ClearSelection();
        ...
        private void RemoveSceneRepresentation()
        {
            if (editorSceneController != null)
            {
        ...
        private void DrawEditLayersTab()
        {
            ...
            if (GUILayout.Button(TEST_LEVEL, EditorCustomStyles.button))
            {
                SetAsCurrentLevel();
            }
            ...
            EditorGUILayout.LabelField(LAYERS_LABEL, EditorCustomStyles.labelMediumBold);
            EditorGUILayout.BeginHorizontal();

            if (selectedLevelRepresentation.layersProperty.arraySize == 1)
            {
                if (GUILayout.Button("Add larger layer", EditorCustomStyles.buttonBlue))
                {
                ...
                if (GUILayout.Button("Add smaller layer", EditorCustomStyles.buttonBlue))
                {
                    selectedLevelRepresentation.AddLayer(-1);
                    UpdateLayersOptions();
                    selectedLayerIndex = 0;
            ...
            else if (GUILayout.Button("Add Layer", EditorCustomStyles.buttonBlue))
            {
            ...
            if (GUILayout.Button("Remove top layer", EditorCustomStyles.buttonRed))
            {
            ...
            if (GUILayout.Button("Clear effects", EditorCustomStyles.button, GUILayout.Width(EditorGUIUtility.labelWidth)))
            {
                selectedLevelRepresentation.ClearEffects();
            }

            if (GUILayout.Button("Trim Level", EditorCustomStyles.button, GUILayout.Width(EditorGUIUtility.labelWidth)))
            {
        ...
        private void DrawEditVariablesTab()
        {
            ...
            if (GUILayout.Button(RETURN_LABEL, EditorCustomStyles.button))
            {
                drawEditVariablesTabDisplayed = false;
            }

            EditorGUILayout.LabelField("Colors used during level editing", EditorCustomStyles.labelMediumBold);
