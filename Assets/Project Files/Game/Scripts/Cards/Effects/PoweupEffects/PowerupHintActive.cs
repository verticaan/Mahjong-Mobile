...
{
    public class PowerupHintActive : CardActiveEffectBase
    {
        ...
        public override void ApplyActive()
        {
            if (!LevelController.IsBusy)
            {
                ...
                if (tileData != null)
                {
                    ...
                    List<TileBehavior> targetTiles =
                        new List<TileBehavior>(LevelController.GetTilesByType(tileData, requiredElementsCount));
                    ...
                    disableTweenCase = Tween.DelayedCall(1.0f, () => { LevelController.SetBusyState(false); });
